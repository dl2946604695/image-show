export async function onRequestPost(context) {
  ...
  try {
    ...
    const apiKey = env.WEGENT_API_KEY;
...
如果你有更具体的问题，欢迎继续问我！`;
    ...
    if (!apiKey) {
      console.log('[DEBUG] No API key found, using mock response');
    ...
    try {
      console.log('[DEBUG] Attempting to call external API...');
      ...
      console.log('[DEBUG] API call succeeded, streaming response...');
      const encoder = new TextEncoder();
    ...
    const responseText = `抱歉，我暂时无法回答你的问题。`;
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      async start(controller) {
        let index = 0;
